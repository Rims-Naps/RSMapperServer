    ...
    protected static int findRoute(int srcX, int srcY, int srcZ, int srcSizeXY, RouteStrategy strategy, boolean findAlternative) {
        ...
        if (!found && !findAlternative) {
            ...
            if (endX == srcX && endY == srcY) {
                ...
                lowestDistance = endX;
                traceY = endY;
                direction = directions[lowestDistance - graphBaseX][traceY - graphBaseY];
                checkX = direction;
                bufferX[steps] = lowestDistance;
                lowestCost = steps + 1;

                for(bufferY[steps] = traceY; lowestDistance != srcX || traceY != srcY; direction = directions[lowestDistance - graphBaseX][traceY - graphBaseY]) {
                    if (checkX != direction) {
