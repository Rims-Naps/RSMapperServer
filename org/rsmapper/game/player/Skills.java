...
public final class Skills implements Serializable {
    ...
    public int getCombatLevel() {
        ...
        int combatLevel = (int)(((double)(defence + hp) + Math.floor((double)(prayer / 2))) * 0.25) + 1;
        double melee = (double)(attack + strength) * 0.325;
        double ranger = Math.floor((double)ranged * 1.5) * 0.325;
        double mage = Math.floor((double)magic * 1.5) * 0.325;
    ...
    public static int getXPForLevel(int level) {
        ...
        for(int lvl = 1; lvl <= level; ++lvl) {
            points = (int)((double)points + Math.floor((double)lvl + 300.0 * Math.pow(2.0, (double)lvl / 7.0)));
            if (lvl >= level) {
                return output;
            }
    ...
    private void refreshXpCounter() {
        this.player.getPackets().sendConfig(1801, (int)(this.xpCounter * 10.0));
    }
    ...
    public static String getColorFormattedNameFor(int skillId) {
        return (exp != null && !(exp[skillId] <= 0.0) ? StatusColor.TRAINED.getColor() : StatusColor.UNTRAINED.getColor()) + SKILL_NAME[skillId];
    }
