    ...
    public void generateAppearenceData() {
        ...
        if (this.transformedNpcId >= 0) {
            ...
            for(int index = 0; index < 4; ++index) {
                ...
                if (this.glowRed) {
                    if (index == 0) {
                        stream.writeShort('?' + ItemEquipIds.getEquipId(2910));
                        continue;
                    }

                    if (index == 1) {
                        stream.writeShort('?' + ItemEquipIds.getEquipId(14641));
                        continue;
                    }
                }
                ...
                if (item == null) {
                    ...
                    stream.writeShort('?' + item.getEquipId());
                }
            }

            Item item = this.player.getEquipment().getItems().get(4);
            stream.writeShort(item == null ? 256 + this.lookI[2] : '?' + item.getEquipId());
            ...
            stream.writeShort(this.glowRed ? '?' + ItemEquipIds.getEquipId(2908) : (item == null ? 256 + this.lookI[5] : '?' + item.getEquipId()));
            ...
            stream.writeShort(this.glowRed ? '?' + ItemEquipIds.getEquipId(2912) : (item == null ? 256 + this.lookI[4] : '?' + item.getEquipId()));
            item = this.player.getEquipment().getItems().get(10);
            stream.writeShort(this.glowRed ? '?' + ItemEquipIds.getEquipId(2904) : (item == null ? 256 + this.lookI[6] : '?' + item.getEquipId()));
