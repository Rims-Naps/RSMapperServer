...
public class Magic {
    ...
    public static final void processLunarSpell(Player player, int spellId, int packetId) {
        ...
        switch (spellId) {
            case 33:
                ...
                player.getSkills().addXp(6, 90.0, new boolean[0]);
                player.lock(3L);
                break;
            case 37:
    ...
    public static final void processAncientSpell(Player player, int spellId, int packetId) {
        switch (spellId) {
            ...
            default:
                break;
            case 40:
                sendAncientTeleportSpell(player, 54, 64.0, new WorldTile(3099, 9882, 0), 563, 2, 554, 1, 556, 1);
                break;
            case 41:
                sendAncientTeleportSpell(player, 60, 70.0, new WorldTile(3222, 3336, 0), 563, 2, 566, 1);
                break;
            case 42:
                sendAncientTeleportSpell(player, 66, 76.0, new WorldTile(3492, 3471, 0), 563, 2, 565, 1);
                break;
            case 43:
                sendAncientTeleportSpell(player, 72, 82.0, new WorldTile(3006, 3471, 0), 563, 2, 555, 4);
                break;
            case 44:
                sendAncientTeleportSpell(player, 78, 88.0, new WorldTile(2990, 3696, 0), 563, 2, 554, 3, 556, 2);
                break;
            case 45:
                sendAncientTeleportSpell(player, 84, 94.0, new WorldTile(3217, 3677, 0), 563, 2, 566, 2);
                break;
            case 46:
                sendAncientTeleportSpell(player, 90, 100.0, new WorldTile(3288, 3886, 0), 563, 2, 565, 2);
                break;
            case 47:
                sendAncientTeleportSpell(player, 96, 106.0, new WorldTile(2977, 3873, 0), 563, 2, 555, 8);
                break;
            case 48:
                sendAncientTeleportSpell(player, 0, 0.0, Constants.HOME_TILE);
        }
    ...
    public static final void processNormalSpell(Player player, int spellId, int packetId) {
        ...
        switch (spellId) {
            ...
            case 88:
                ...
                player.lock(4L);
                player.setActionTime(player.getActionTime() + TimeUnit.SECONDS.toMillis((long)Utils.random(5, 20)));
                player.getInterfaceManager().openGameTab(7);
                player.getInventory().deleteItem(target.getId(), 1);
                player.getSkills().addXp(6, (double)(highAlch ? 65 : 31), new boolean[0]);
                boolean ancientStat = target.getName().toLowerCase().equalsIgnoreCase("ancient statuette");
                player.getInventory().addItem(new Item(995, ancientStat ? 1000000 : (int)((double)target.getDefinitions().getValue() * (highAlch ? 0.6 : 0.3))));
                ...
                break;
            case 37:
                sendNormalTeleportSpell(player, 10, 19.0, new WorldTile(2413, 2848, 0), 563, 1, 555, 1, 556, 1);
                break;
            case 40:
                sendNormalTeleportSpell(player, 25, 19.0, new WorldTile(3212, 3424, 0), 554, 1, 556, 3, 563, 1);
                break;
            case 43:
                sendNormalTeleportSpell(player, 31, 41.0, new WorldTile(3222, 3218, 0), 557, 1, 556, 3, 563, 1);
                break;
            case 46:
                sendNormalTeleportSpell(player, 37, 48.0, new WorldTile(2964, 3379, 0), 555, 1, 556, 3, 563, 1);
                break;
            case 51:
                sendNormalTeleportSpell(player, 45, 55.5, new WorldTile(2757, 3478, 0), 556, 5, 563, 1);
                break;
            case 57:
                sendNormalTeleportSpell(player, 51, 61.0, new WorldTile(2664, 3305, 0), 555, 2, 563, 2);
                break;
            case 62:
                sendNormalTeleportSpell(player, 58, 68.0, new WorldTile(2547, 3113, 2), 557, 2, 563, 2);
                break;
            case 69:
                sendNormalTeleportSpell(player, 61, 68.0, new WorldTile(2888, 3674, 0), 554, 2, 563, 2);
                break;
            case 72:
                sendNormalTeleportSpell(player, 64, 76.0, new WorldTile(2776, 9103, 0), 554, 2, 555, 2, 563, 2, 1963, 1);
        }
    ...
    private static void useHomeTele(Player player) {
        ...
        if (!player.isLocked() && player.getAttackedByDelay() + 10000L <= currentTime && player.getEmotesManager().getNextEmoteEnd() < currentTime) {
            sendNormalTeleportSpell(player, 0, 0.0, Constants.HOME_TILE);
        } else {
    ...
    public static final void sendRainbowTeleportSpell(Player player, WorldTile tile) {
        sendTeleportSpell(player, 1979, -1, 6, -1, 1, 0.0, tile, 2, false, 0);
    }
    ...
    public static final void sendPurpleTeleportSpell(Player player, WorldTile tile) {
        sendTeleportSpell(player, 1816, -2, 342, -1, 1, 0.0, tile, 4, false, 0);
    }

    public static final void sendGreenTeleportSpell(Player player, WorldTile tile) {
        sendTeleportSpell(player, 3254, 3255, 2670, 2671, 1, 0.0, tile, 3, false, 0);
    }

    public static final void sendRingTeleportSpell(Player player, WorldTile tile) {
        sendTeleportSpell(player, 9603, -2, 1684, -1, 1, 0.0, tile, 3, false, 0);
    }
    ...
    public static final boolean sendItemTeleportSpell(Player player, boolean randomize, int upEmoteId, int upGraphicId, int delay, WorldTile tile) {
        return sendTeleportSpell(player, upEmoteId, -2, upGraphicId, -1, 0, 0.0, tile, delay, randomize, 1);
    }
    ...
    public static final void sendObjectTeleportSpell(Player player, boolean randomize, WorldTile tile) {
        sendTeleportSpell(player, 8939, 8941, 1576, 1577, 0, 0.0, tile, 3, randomize, 2);
    }

    public static final void sendDelayedObjectTeleportSpell(Player player, int delay, boolean randomize, WorldTile tile) {
        sendTeleportSpell(player, 8939, 8941, 1576, 1577, 0, 0.0, tile, delay, randomize, 2);
    }
        ...
        if (player.getLockDelay() > currentTime) {
            ...
            WorldTasksManager.schedule(new WorldTask() {
                public void run() {
                    ...
                    if (xp != 0.0) {
                        player.getSkills().addXp(6, xp, new boolean[0]);
                    }
